...
parser.add_argument('--testnotify', dest='testnotify', action='store_true', help="test telegram notify") #aggiunta mia
parser.set_defaults(testnotify=False) #aggiunta mia
                ...
                for minP in url[1].items():
                    for maxP in minP[1].items():
                        for result in maxP[1].items():
                            print("\n")
                            print(result[1].get('title').encode("utf-8", errors="ignore"))
                            print(" Price:", result[1].get('price'))
                            print(" Location:", result[1].get('location'))
                            print(" Link:", result[0])
